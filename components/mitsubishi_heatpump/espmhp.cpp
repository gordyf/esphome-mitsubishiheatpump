...
MitsubishiHeatPump::MitsubishiHeatPump(
    ...
    uint32_t poll_interval) : PollingComponent{poll_interval}, // member initializers list
...
void MitsubishiHeatPump::set_function_code(int code, int value)
{
    ...
    {
        ESP_LOGE(TAG, "Failed to set code %d to value %d in fetched function codes.", code, value);
        return;
    }
    ...
    ESP_LOGI(TAG, "Function codes updated successfully.");
}
